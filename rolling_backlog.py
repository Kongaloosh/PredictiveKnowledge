...
class MinecraftHordeHolder(HordeHolder):
    ...
    def step(self, observations, policy=None, action=None, recurrance=False, skip=False, ude=False, remove_base=False, **vals):
        ...
        self.last_prediction = all_predictions
...
class Foreground:
    ...
    def __init__(self, show_display=True, steps_before_updating_display=0, steps_before_prompting_for_action=0):
        ...
        self.action_count = 0
        self.action_buffer = [None] * 6
        self.actions = [None] * 6
        self.tracking_predictions_buffer = [None] * 6
        self.tracking_predictions = [None] * 6
        self.exp_predictions_buffer = [None] * 6
        self.exp_predictions = [None] * 6
        self.env_t = [None] * 6
        self.env_tl = [None] * 6
        self.env_tr = [None] * 6
        self.prediction_rupee_buffer = [None] * 6
        self.prediction_ude_buffer = [None] * 6
        self.tracking_rupee_buffer = [None] * 6
        self.tracking_ude_buffer = [None] * 6

        self.vingettes_left = []
        self.vingettes_right = []
    ...
    def learn_from_behavior_policy_action(self):
        ...
        self.exp_predictions_buffer.pop(0)
        self.exp_predictions_buffer.append(self.network.last_prediction)

        self.tracking_predictions_buffer.pop(0)
        self.tracking_predictions_buffer.append(self.tracking_network.last_prediction)

        vals = self.get_true_values()
        self.env_t.pop(0)
        self.env_t.append(vals[0][0])

        self.env_tl.pop(0)
        self.env_tl.append(vals[1][0])

        self.env_tr.pop(0)
        self.env_tr.append(vals[1][1])

        self.prediction_rupee_buffer.pop(0)
        self.prediction_rupee_buffer.append(self.network.rupee)

        self.tracking_rupee_buffer.pop(0)
        self.tracking_rupee_buffer.append(self.tracking_network.rupee)

        self.prediction_ude_buffer.pop(0)
        self.prediction_ude_buffer.append(self.network.ude)

        self.tracking_ude_buffer.pop(0)
        self.tracking_ude_buffer.append(self.tracking_network.ude)

        # 2 random, forwards, blap, turn l/r, 2 random
        if np.array_equal([0, 3, 1], self.action_buffer[2:5]):
            #     This  needs to be stored
            self.vingettes_left.append(
                {
                    "predictions" : self.exp_predictions_buffer,
                    "tracker" : self.tracking_predictions_buffer,
                    "t": self.env_t,
                    "tl": self.env_tl,
                    "tr": self.env_tr,
                    "actions": self.action_buffer,
                    "rupee_prediction": self.prediction_rupee_buffer,
                    "rupee_tracking": self.tracking_rupee_buffer,
                    "ude_prediction": self.prediction_ude_buffer,
                    "ude_tracking": self.tracking_ude_buffer
                 }
            )

        if np.array_equal([0, 3, 2], self.action_buffer[2:5]):
            self.vingettes_right.append(
                {
                    "predictions": self.exp_predictions_buffer,
                    "tracker": self.tracking_predictions_buffer,
                    "t": self.env_t,
    ...
    def start(self):
        ...
        for seed in [
            ...
            self.tracking_network = self.configure_tracking_gvfs()
            print("\nSEED FINISHED {0}; {1} Vingettes!\n".format(seed, len(self.vingettes_right)+len(self.vingettes_left)))
        with open('results/vingette_results_1.pkl', "wb") as f:
            pickle.dump(
                {
                    "vingette_left": self.vingettes_left,
                    "vingette_right": self.vingettes_right
                },
                f
            )
